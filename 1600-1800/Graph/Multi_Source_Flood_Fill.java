...
public class Multi_Source_Flood_Fill {
     public int[][] colorGrid(int n, int m, int[][] sources) {
        ...
        for(int i=0; i<sources.length; i++) {
            q.add(new Group(sources[i][0],sources[i][1],sources[i][2]));
            ans[sources[i][0]][sources[i][1]]=sources[i][2];
        }
        ArrayList<Group> temp=new ArrayList<>();
        int size=q.size();
        for(int i=0; i<size; i++) temp.add(q.remove());
        temp.sort((a,b)->b.val-a.val);
        for(int i=0; i<temp.size(); i++) q.add(temp.get(i));
        while(!q.isEmpty()){
            Group group=q.remove();
            int parentrow=group.row;
            int parentcol=group.col;
            int parentval=group.val;
            int dir[][]={{1,0},{-1,0},{0,-1},{0,1}};
            for(int i=0; i<4; i++){
                int row=parentrow+dir[i][0];
                int col=parentcol+dir[i][1];
                if(row<0 || row>=n || col<0 || col>=m || ans[row][col]!=0) continue;
                ans[row][col]=parentval;
                q.add(new Group(row, col, parentval));
            }
        }
        return ans;
    }
